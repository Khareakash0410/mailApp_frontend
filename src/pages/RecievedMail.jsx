...
import { ArrowBigDownDash, BellPlus, BookType, MessageSquareText, Paperclip, Trash2, X } from 'lucide-react';
...
let url = import.meta.env.VITE_DEPLOYEMENT === "production" ? import.meta.env.VITE_ENDPOINT : "http://localhost:8080"
  ...
  const [recieveMails, setRecieveMails] = useState([]);
  ...
  const currentMails = recieveMails.slice(indexOfFirstMail, indexOfLastMail);



  const totalPages = Math.ceil(recieveMails.length / itemsPerPage);
  ...
  const prevPage = () => {
   ...
   async function getRecieveMails() {

    try {
        let res = await axios.get(`${url}/mails/recieveMails`, {withCredentials: true, headers : {
            "Authorization": User?.token
        }});

        if(res.status === 200 || 201) {
            setRecieveMails(res.data.recieveMails);
        }
    } catch (error) {
    ...
    const deleteSelectedMails = async () => {
        try {
            for (const _id of selectedMails) {
                await axios.delete(`${url}/mails/delete/recieveMail/${_id}`, {
                    withCredentials: true,
                    headers: { "Authorization": User?.token }
                });
            }
            setRecieveMails((prevMails) => prevMails.filter((mail) => !selectedMails.includes(mail._id)));
            setSelectedMails([]);
...
<div className="p-4 flex flex-col sm:ml-48">
...

    {
        latestMail && latestMail.map((ele, index) => {
            return (
                <Link to={`/singleMail`} state={{mail: ele}} key={index} className="p-4 bg-gray-800 flex flex-col gap-1 w-full shadow-lg border-[1px]  border-green-500 rounded-lg mt-14 relative transition-transform duration-300 hover:scale-y-110">
