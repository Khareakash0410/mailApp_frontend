...
import { ArrowBigDownDash, BellPlus, BookType, CornerRightUp, MessageSquareText, Paperclip, Trash2, X } from 'lucide-react';
...
const totalPages = Math.ceil(allMails.length / itemsPerPage);
...
async function getAllMails() {
    ...
    try {
        ...
        if(res.status === 200 || 201) {
            setAllMails(res.data.findMails);
        }
    } catch (error) {
 ...
 const deleteSelectedMails = async () => {
    try {
        for (const _id of selectedMails) {
            await axios.delete(`${url}/mails/delete/mail/${_id}`, {
                withCredentials: true,
                headers: { "Authorization": User?.token }
            });
        }
        setAllMails((prevMails) => prevMails.filter((mail) => !selectedMails.includes(mail._id)));
        setSelectedMails([]);
...
useEffect(() => {
socket.on("recieveMail", (obj) => {
    setLatestMail((prevMails) => [obj, ...prevMails]);
})
}, [socket]);
...
<div className="p-4 flex flex-col sm:ml-48">
    ...
    {
        latestMail && latestMail.map((ele, index) => {
            return  (
                <Link to={`/singleMail`} key={index} state={{mail: ele}} className="p-4 relative bg-gray-800 flex flex-col gap-1 shadow-lg border-[1px]  border-green-500 rounded-lg mt-14  transition-transform duration-300 hover:scale-y-110">

                       <BellPlus size={20} color="yellow" className="absolute bg-gray-800 right-1 top-1" />
           
                         {
                            ele?.from === User?.userDetails.email ? 
                            (
                                ...
                                <span className='text-lg font-serif text-yellow-100 bg-gray-800 truncate flex-grow'>{ele?.to}</span>
                                </div>
                            ) 
                            : 
                            (
                                ...
                                <span className='text-lg font-serif text-yellow-100 bg-gray-800 truncate flex-grow'>{ele?.from}</span>
                                </div>
                            )
                         }
...

    {
        currentMails?.map((element, index) => {
            return (
                <Link to={`/singleMail?id=${element._id}`} state={element._id} key={index} className="p-4 bg-gray-800 flex flex-col gap-1 shadow-lg border-[1px]  border-green-500 rounded-lg mt-14 relative  transition-transform duration-300 hover:scale-y-110">
                     ...
                     {
                        element?.from === User?.userDetails.email ? 
                        (
                            <div className='bg-gray-800 flex items-center gap-1 sm:gap-3 '>
                            <CornerRightUp color='yellow' size={20} className='bg-gray-800 shrink-0 flex-none' />
                        ...
                        (
                            <div className='bg-gray-800 flex items-center gap-1 sm:gap-3 '>
                            <ArrowBigDownDash color='yellow' size={20} className='bg-gray-800 shrink-0 flex-none' />
                            <span className='text-lg font-serif text-yellow-100 bg-gray-800 truncate flex-grow'>{element?.from}</span>
                            </div>
                        )
                     }
